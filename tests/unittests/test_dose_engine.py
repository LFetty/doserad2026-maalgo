...
# ---------------------------------------------------------------------------
# Sparse-direct photon backend tests
# ---------------------------------------------------------------------------

_SPARSE_CT_SHAPE = (12, 12, 12)
_SPARSE_RESOLUTION = (4.0, 4.0, 4.0)
_SPARSE_FIELD_SIZE = (80, 80)
_SPARSE_KERNEL_SIZE = 9


def _configure_multi_cp_sequence(sequence: BeamSequence) -> BeamSequence:
    with torch.no_grad():
        sequence.leaf_positions[0, :, 0] = -8.0
        sequence.leaf_positions[0, :, 1] = 10.0
        sequence.leaf_positions[1, :, 0] = -5.0
        sequence.leaf_positions[1, :, 1] = 7.0
        sequence.jaw_positions[0] = torch.tensor((-10.0, 10.0), device=sequence.device, dtype=sequence.dtype)
        sequence.jaw_positions[1] = torch.tensor((-6.0, 6.0), device=sequence.device, dtype=sequence.dtype)
        sequence.mus[0] = 0.75
        sequence.mus[1] = 1.25
    return sequence


def _max_value(tensor: torch.Tensor) -> torch.Tensor:
    return tensor.detach().max()


def _sum_value(tensor: torch.Tensor) -> torch.Tensor:
    return tensor.detach().sum()
