...
from pydose_rt.data import MachineConfig
from pydose_rt.geometry.rotations import build_rotation_grids, get_radiological_depth_indices
...
class RadiologicalDepthLayer(nn.Module):
    ...
    def __init__(self, 
        ...
        # Inverse rotation grid for full-BEV density sampling (patient → BEV)
        H, D, W = ct_array_shape
        G = len(gantry_angles)
        angles_tensor = torch.as_tensor(gantry_angles, device=self.device, dtype=self.dtype)
        inv_depth_origin = "entry_bev_to_patient" if depth_origin == "entry" else depth_origin
        inv_grid = build_rotation_grids(
            (1, G, D, H, W),
            -angles_tensor,
            self.device,
            self.dtype,
            iso_center=iso_center,
            resolution=resolution,
            depth_origin=inv_depth_origin,
        )  # [1, G, 1, D, W, 2]
        self.register_buffer("_inv_rot_grid", inv_grid)
    ...
    def forward_bev(self, ct_stack: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Compute per-voxel density and WEQ in BEV coordinates.

        Inverse-rotates the patient density volume into each beam's BEV frame,
        then computes WEQ per column via cumulative sum along depth.  This
        matches the sparse backend where every fan ray gets its own WEQ.

        Args:
            ct_stack: ``[B, H, D, W]`` patient-frame density (RSP).

        Returns:
            ``(density_bev, weq_bev)`` each ``[B*G, D, H, W]`` in BEV layout.
        """
        with torch.no_grad():
            B, H, D, W = ct_stack.shape
            G = self._inv_rot_grid.shape[1]

            ct_flat = ct_stack.reshape(B * H, 1, D, W)

            density_bev_parts = []
            for g in range(G):
                grid_g = self._inv_rot_grid[0, g, 0]  # [D, W, 2]
                grid_g = grid_g.unsqueeze(0).expand(B * H, -1, -1, -1)
                den_g = F.grid_sample(
                    ct_flat, grid_g,
                    mode="bilinear", padding_mode="zeros", align_corners=False,
                )  # [B*H, 1, D, W]
                den_g = den_g.reshape(B, H, D, W)
                density_bev_parts.append(den_g)

            # [B, G, H, D, W] → BEV layout [B, G, D, H, W]
            density_bev = torch.stack(density_bev_parts, dim=1)
            density_bev = density_bev.permute(0, 1, 3, 2, 4).contiguous()

            # WEQ per column — matching sparse convention:
            # center_depth = boundary_end - 0.5 * segment_weq
            ...
            return (
                density_bev.reshape(B * G, D, H, W),
                weq_bev.reshape(B * G, D, H, W),
            )
