...
class MachineConfig(BaseSettings):
    ...
    fit_air_offset_mm: float = Field(
        default=0.0,
        description=(
            "Air distance (mm, water-equivalent units irrelevant — this is geometric) "
            "already included when the proton pencil-beam kernels were fitted. "
            "Used by the sparse-direct layer to compute rad_depth_offset = "
            "0.0011 * ((SSD + bams_to_iso_dist) - SAD - fit_air_offset_mm)."
        ),
    )
    bams_to_iso_dist_mm: float = Field(
        default=1000.0,
        description=(
            "Geometric distance from the beam-monitoring-system (nozzle exit) to "
            "iso-center, in mm. Default matches pyRadPlan's Generic protons machine."
        ),
    )
